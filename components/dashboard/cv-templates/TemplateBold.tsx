...
export default function TemplateBold({ cv, matchedKeywords = [] }: Props) {
  ...
  return (
    <div className="font-sans text-sm min-h-[1000px]" style={{ background: '#FFFFFF' }}>
      <div className="p-7" style={{ background: '#1A1A2E', color: textColor }}>
        ...
        <p className="text-sm font-medium mb-2" style={{ color: '#F4A97F' }}>{personal.jobTitle}</p>
      ...
      <div className="p-7 space-y-5 bg-white">
        {personal.summary && (
          <section>
            <h2 className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: '#F4A97F' }}>Summary</h2>
            <p className="text-gray-600 leading-relaxed">{personal.summary}</p>
          </section>
        )'
        {experience.length > 0 && (
          <section>
            <h2 className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: '#F4A97F' }}>Experience</h2>
        ...
        {education.length > 0 && (
          <section>
            <h2 className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: '#F4A97F' }}>Education</h2>
            {education.map((edu) => (
        ...
        {skills.length > 0 && (
          <section>
            <h2 className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: '#F4A97F' }}>Skills</h2>
            <div className="flex flex-wrap gap-1.5">
              {skills.slice(0,10).map(s => (
                <span key={s} className="text-xs border px-2 py-0.5 rounded-full text-gray-600" style={{ borderColor: '#F4A97F40' }}>{s}</span>
              ))}
            </div>
          </section>
        )'
